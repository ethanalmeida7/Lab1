    ...
    public class IndexModel : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            try
            {
                ...
                string lessonQuery = "SELECT S.FirstName + ' ' + S.LastName AS StudentName, T.Names AS TutorName, L.Dates, L.Times, L.Duration, L.PracticeTopics " +
                                     "FROM Lessons L " +
                ...
                while (lessonReader.Read())
                {
                    Lessons.Add(new Lesson
                    {
                        ...
                        Date = (DateTime)lessonReader["Date"],
                        Time = (TimeSpan)lessonReader["Time"],
                        Duration = (int)lessonReader["Duration"],
                        Subject = lessonReader["Subject"].ToString()
                    });
                }
                lessonReader.Close();

                // Fetch group reservations
                string reservationQuery = "SELECT G.GroupName, R.Locations AS RoomName, Res.Date, Res.Time, Res.Duration, Res.Description " +
